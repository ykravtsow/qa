...
def parse_log(str):
    ...
    for line in str:
        ...
        t1 = tstr.find('"')
        if t1 == -1:
            l.error('error in line ' + str(n) + ' in duration part')
            d = d + 1
            continue
        tstr = tstr[t1+2:]
        t1 = tstr.find('\n')
        ...
        arr.append((ip, ts, method, url, sc, dur))
        n = n + 1
...
def get_top_longs(r):
    ...
    longs.clear()
    for i in range(0, len(tmp)):
        longs.append((tmp[i][0][2], tmp[i][0][3], tmp[i][0][4], tmp[i][0][5]))
    return longs
...
if not os.path.isdir(d):
    l.error('directory not exists ('+d+')')
    sys.exit(-1)
